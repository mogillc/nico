...
void printUsage( char *argv[] )
{
	...
	printf("Usage:\n");
	printf("   %s [-d] [-j JSONFILE]\n", argv[0]);
	printf("\n");
	//printf("  -s:    Simulation enable (true hexapreadyod control)\n");
	printf("  -d:    daemon mode (will not use ncurses) \n");
	printf("  -j:    the json file in the resources directory \n");
	printf("\n");
}
...
void interruptHandler( int signal ) {
	std::cout << "Received: " << strsignal(signal) << std::endl;
	keepRunning = false;
}
...
int main(int argc, char* argv[])
{
	...
	if( signal(SIGINT, interruptHandler) == SIG_ERR )	// ctrl-c kills
	{
		std::cerr << "Unable to catch SIGINT!" << std::endl;
	}
	if( signal(SIGKILL, interruptHandler) == SIG_ERR )
	{
//		std::cerr << "Unable to catch SIGKILL!" << std::endl;
	}
	if( signal(SIGSTOP, interruptHandler) == SIG_ERR )
	{
//		std::cerr << "Unable to catch SIGSTOP!" << std::endl;
	}
	if( signal(SIGTERM, interruptHandler) == SIG_ERR )	// for systemd killing
	{
	...
	mCursesHandler->setupTerminal( daemonMode );
	...
	while(!terminate){
		...
		// ncurses handling:
		terminate = danceState.dealWithInput(mCursesHandler->getUserInput(), &hex);

		danceState.checkLimits();
		cursesUpdateTracker += time.dTime();
		if (cursesUpdateTracker > 0.2) {
			mCursesHandler->updateScreen(&danceState);
			cursesUpdateTracker = 0;
		}
...
//				std::map<unsigned char, Motor*> dynamixels = hex.dyn;
...
//				}
		...
		if (danceState.quaternionMagnitude > 0) {
			...
			headAngles(0) =  ((26.506 - 3.7601)/2.0 + .5 *(26.506 - 3.7601)*sin(newFrequency * 2.0*MOGI_PI * time.runningTime()*1.1)) * MOGI_PI/180.0;
			headAngles(1) = 0.9 * 20.0 * sin(newFrequency * 2.0*MOGI_PI * time.runningTime() +MOGI_PI/2) * MOGI_PI/180.0;
			headAngles(2) = .6 * 13.0 * sin(newFrequency * 2.0*MOGI_PI * time.runningTime()) * MOGI_PI/180.0;
			...
			bodyOffset(2) = camOffset(2,0) + 65;;// + bodyheight;
		...
		if (danceState.fistPumpMagnitude != 0) {
			...
			if(hex.isIdle())
			{
				hex.beginModifying();
				{
					...
					Vector footXYCenter = 57.5 *sqrt(2)/2 *additionalRotation.makeRotationMatrix() * Vector::xAxis;
					//hex.foot[1](0) = footXYCenter(0) + 40*cos(timePass*32.0);
					//hex.foot[1](1) = footXYCenter(1) + 40*sin(timePass*32.0);
					footLocation(0) = footXYCenter(0) + 40*cos(timePass*32.0);
					footLocation(1) = footXYCenter(1) + 40*sin(timePass*32.0);

					//					hex.foot[1] *= danceState.fistPumpMagnitude;
					//					hex.foot[1] += footSave;
					footLocation *= danceState.fistPumpMagnitude;
					//footLocation += footSave;
					hex.setFootRelativeToRest(1, footLocation);
				}
				hex.endModifying();
			}
		}
