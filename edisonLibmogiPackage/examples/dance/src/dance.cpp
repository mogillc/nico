...
{
    Hexapod hex;                // hexapod class to operate hexapod
...
//		printf("Woot! Openned a MIDI device!\n");
    ...
    Time time;                  // This keeps track of time, see main loop below
    ...
    while(!terminate){
		...
		if (sixaxisEnable) {
			...
			//hex.foot[1](2) = 10 + hex.footinit[1](2) + time.runningtime*480/40;
            //hex.foot[1](0) = hex.footinit[1](0) + 40 + 40*cos(3*time.runningtime);
            //hex.foot[1](1) = hex.footinit[1](1) + 40 + 40*sin(3*time.runningtime);
		}
        ...
        hex.beginModifying();
        {
			if (hex.walking())
			{
				...
				hex.setBodyAngularVelocity( bodyAngularVelocity );
			}
        }
        hex.endModifying();
