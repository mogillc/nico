...
int read_decode_eeprom(struct ftdi_context *ftdi) {
  ...
  for (i = 0; i < size; i += 16) {
    ...
    for (j = 0; j < 8; j++) fprintf(stdout, " %02x", buf[i + j]);
    fprintf(stdout, " ");
    for (; j < 16; j++) fprintf(stdout, " %02x", buf[i + j]);
    fprintf(stdout, " ");
    for (j = 0; j < 8; j++)
...
int main(int argc, char **argv) {
  ...
  while ((i = getopt(argc, argv, "d::ev:p:l:P:S:w")) != -1) {
    switch (i) {
      ...
      default:
        ...
        fprintf(stderr,
                "\t-d[num] Work with default valuesfor 128 Byte "
        ...
        fprintf(stderr,
                "\t-P <string? Search for device with given "
                "product description\n");
        fprintf(stderr,
                "\t-S <string? Search for device with given "
                "serial number\n");
        retval = -1;
        goto done;
    }
  ...
  if (!vid && !pid && desc == NULL && serial == NULL) {
    ...
    if (res > 1) {
      ...
      for (curdev = devlist; curdev != NULL; curdev = curdev->next, i++) {
        ...
        if (f < 0) {
          fprintf(stderr, "Unable to open device %d: (%s)", i,
                  ftdi_get_error_string(ftdi));
          continue;
        }
    ...
    if (f < 0) {
      ...
      if (desc) fprintf(stderr, " Desc %s", desc);
      if (serial) fprintf(stderr, " Serial %s", serial);
      fprintf(stderr, "\n");
  ...
  if (erase) {
    ...
    if (f < 0) {
      fprintf(stderr, "Erase failed: %s", ftdi_get_error_string(ftdi));
      retval = -2;
      goto done;
    }
