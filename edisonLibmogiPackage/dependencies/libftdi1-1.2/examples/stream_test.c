...
static int readCallback(uint8_t *buffer, int length, FTDIProgressInfo *progress,
                        void *userdata) {
  if (length) {
    if (check) {
      int i, rem;
      ...
      for (i = offset; i < length - 16; i += 16) {
        ...
        if (start && (num != start + 0x4000)) {
          ...
          fprintf(stderr,
                  "Skip %7d blocks from 0x%08x to 0x%08x at blocks %10llu\n",
  ...
  if (progress) {
    fprintf(
        stderr,
        "%10.02fs total time %9.3f MiB captured %7.1f kB/s curr rate %7.1f "
        "kB/s totalrate %d dropouts\n",
...
int main(int argc, char **argv) {
  ...
  if (outfile)
    if ((of = fopen(outfile, "w+")) == 0)
      fprintf(stderr, "Can't open logfile %s, Error %s\n", outfile,
              strerror(errno));
  if (of)
    if (setvbuf(of, NULL, _IOFBF, 1 << 16) == 0) outputFile = of;
  signal(SIGINT, sigintHandler);

  err = ftdi_readstream(ftdi, readCallback, NULL, 8, 256);
  if (err < 0 && !exitRequested) exit(1);
  ...
  if (check && outfile) {
    ...
    fprintf(stderr, "%d errors of %llu blocks (%Le), %d (%Le) blocks skipped\n",
            n_err, (unsigned long long)blocks,
            (long double)n_err / (long double)blocks, skips,
  ...
  if (strcmp(descstring, "FT2232HTEST") == 0) {
    ...
    while (fgets(pb, 1023, outputFile) != NULL) {
      ...
      if (sscanf(pa, "%6u%94s%6u", &num_start, bufr, &num_end) != 3) {
        fprintf(stdout, "Format doesn't match at line %8d \"%s", num_lines, pa);
        err_count++;
        line_num = num_save + 2;
      } else {
