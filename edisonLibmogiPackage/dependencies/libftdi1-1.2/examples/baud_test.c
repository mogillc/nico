 ...
 *  -p <devicestring> defaults to "i:0x0403:0x6001" (this is the first FT232R
 with default id)
 *       d:<devicenode> path of bus and device-node (e.g. "003/001") within usb
 device tree (usually at /proc/bus/usb/)
 *       i:<vendor>:<product> first device with given vendor and product id,
 *                            ids can be decimal, octal (preceded by "0") or hex
 (preceded by "0x")
...
int main(int argc, char **argv) {
  ...
  // prepare data to send: 0 and 1 bits alternating (except for serial
  // start/stopbit):
  ...
  while (i < datasize) {
    ...
    if (i + sendsize > datasize) sendsize = datasize - i;
  ...
  printf("and took %.4f seconds, this is %.0f baud or factor %.3f\n", duration,
         (plan * baud) / duration, plan / duration);
do_close:
  ftdi_usb_close(ftdi);
do_deinit:
  ftdi_free(ftdi);
