...
static void ftdi_usb_close_internal(struct ftdi_context *ftdi) {
  if (ftdi && ftdi->usb_dev) {
    ...
    ftdi->usb_dev = NULL;
    if (ftdi->eeprom) ftdi->eeprom->initialized_for_connected_device = 0;
  }
}
...
int ftdi_init(struct ftdi_context *ftdi) {
  struct ftdi_eeprom *eeprom =
      (struct ftdi_eeprom *)malloc(sizeof(struct ftdi_eeprom));
  ftdi->usb_ctx = NULL;
  ftdi->usb_dev = NULL;
  ...
  if (eeprom == 0) ftdi_error_return(-2, "Can't malloc struct ftdi_eeprom");
...
int ftdi_set_interface(struct ftdi_context *ftdi,
  ...
  if (ftdi == NULL) ftdi_error_return(-2, "USB device unavailable");

  if (ftdi->usb_dev != NULL) {
    int check_interface = interface;
    if (check_interface == INTERFACE_ANY) check_interface = INTERFACE_A;

    if (ftdi->index != check_interface)
      ftdi_error_return(
          -3, "Interface can not be changed on an already open device");
  }
  ...
  while ((dev = devs[i++]) != NULL) {
    ...
    if (libusb_get_device_descriptor(dev, &desc) < 0)
      ftdi_error_return_free_device_list(
          -6, "libusb_get_device_descriptor() failed", devs);

    if (((vendor || product) && desc.idVendor == vendor &&
         desc.idProduct == product) ||
        (!(vendor || product) && (desc.idVendor == 0x403) &&
         (desc.idProduct == 0x6001 || desc.idProduct == 0x6010 ||
...
int ftdi_usb_get_strings(struct ftdi_context *ftdi, struct libusb_device *dev,
  ...
  if ((ftdi == NULL) || (dev == NULL)) return -1;
  ...
  if (manufacturer != NULL) {
    if (libusb_get_string_descriptor_ascii(ftdi->usb_dev, desc.iManufacturer,
  ...
  if (serial != NULL) {
    if (libusb_get_string_descriptor_ascii(ftdi->usb_dev, desc.iSerialNumber,
...
static unsigned int _ftdi_determine_max_packet_size(struct ftdi_context *ftdi,
  ...
  if (libusb_get_device_descriptor(dev, &desc) < 0) return packet_size;

  if (libusb_get_config_descriptor(dev, 0, &config0) < 0) return packet_size;
...
int ftdi_usb_open_dev(struct ftdi_context *ftdi, libusb_device *dev) {
  ...
  if (ftdi == NULL) ftdi_error_return(-8, "ftdi context invalid");
  ...
  if (desc.bNumConfigurations > 0 && cfg != cfg0) {
    if (libusb_set_configuration(ftdi->usb_dev, cfg0) < 0) {
      ...
      if (detach_errno == EPERM) {
        ...
        ftdi_error_return(-3,
                          "unable to set usb configuration. Make sure the "
  ...
  if (libusb_claim_interface(ftdi->usb_dev, ftdi->interface) < 0) {
    ...
    if (detach_errno == EPERM) {
      ...
      ftdi_error_return(-5,
                        "unable to claim usb device. Make sure the default "
...
int ftdi_usb_open_desc_index(struct ftdi_context *ftdi, int vendor, int product,
  ...
  if (ftdi == NULL) ftdi_error_return(-11, "ftdi context invalid");
  ...
  while ((dev = devs[i++]) != NULL) {
    ...
    if (libusb_get_device_descriptor(dev, &desc) < 0)
      ftdi_error_return_free_device_list(
          -13, "libusb_get_device_descriptor() failed", devs);
    ...
    if (desc.idVendor == vendor && desc.idProduct == product) {
      ...
      if (description != NULL) {
        if (libusb_get_string_descriptor_ascii(ftdi->usb_dev, desc.iProduct,
          ...
          ftdi_usb_close_internal(ftdi);
          ftdi_error_return_free_device_list(
              -8, "unable to fetch product description", devs);
        }
      ...
      if (serial != NULL) {
        if (libusb_get_string_descriptor_ascii(
                ftdi->usb_dev, desc.iSerialNumber, (unsigned char *)string,
                sizeof(string)) < 0) {
          ftdi_usb_close_internal(ftdi);
          ftdi_error_return_free_device_list(
              -9, "unable to fetch serial number", devs);
        }
...
int ftdi_usb_open_string(struct ftdi_context *ftdi, const char *description) {
  if (ftdi == NULL) ftdi_error_return(-12, "ftdi context invalid");
  ...
  if (description[0] == 'd') {
    ...
    if (sscanf(description + 2, "%u/%u", &bus_number, &device_address) != 2)
      ftdi_error_return_free_device_list(-11, "illegal description format",
                                         devs);
    ...
    if (description[0] == 'i' && *endp != 0) {
      ...
      if (*endp != ':') ftdi_error_return(-11, "illegal description format");
...
int ftdi_usb_reset(struct ftdi_context *ftdi) {
  ...
  if (libusb_control_transfer(ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
                              SIO_RESET_REQUEST, SIO_RESET_SIO, ftdi->index,
...
int ftdi_usb_close(struct ftdi_context *ftdi) {
  ...
  if (ftdi == NULL) ftdi_error_return(-3, "ftdi context invalid");

  if (ftdi->usb_dev != NULL)
    if (libusb_release_interface(ftdi->usb_dev, ftdi->interface) < 0) rtn = -1;
  ...
  if ((ftdi->type == TYPE_2232H) || (ftdi->type == TYPE_4232H) ||
    ...
    if (baudrate * 10 > H_CLK / 0x3fff) {
      /* On H Devices, use 12 000 000 Baudrate when possible
         ...
         Assume AN_120 "Sub-integer divisors between 0 and 2 are not allowed"
         holds for
...
int ftdi_set_baudrate(struct ftdi_context *ftdi, int baudrate) {
  ...
  if (actual_baudrate <= 0) ftdi_error_return(-1, "Silly baudrate <= 0.");
  ...
  if ((actual_baudrate * 2 < baudrate /* Catch overflows */) ||
    ...
    ftdi_error_return(-1,
                      "Unsupported baudrate. Note: bitbang baudrates are "
...
int ftdi_write_data(struct ftdi_context *ftdi, const unsigned char *buf,
  ...
  while (offset < size) {
    ...
    if (libusb_bulk_transfer(ftdi->usb_dev, ftdi->in_ep,
                             (unsigned char *)buf + offset, write_size,
                             &actual_length, ftdi->usb_write_timeout) < 0)
...
static void LIBUSB_CALL ftdi_read_data_cb(struct libusb_transfer *transfer) {
  struct ftdi_transfer_control *tc =
      (struct ftdi_transfer_control *)transfer->user_data;
  ...
  if (actual_length > 2) {
    ...
    if (actual_length > 0) {
      ...
      if (tc->offset + actual_length <= tc->size) {
        memcpy(tc->buf + tc->offset, ftdi->readbuffer + ftdi->readbuffer_offset,
               actual_length);
        ...
        /* printf("Returning part: %d - size: %d - offset: %d - actual_length:
        %d - remaining: %d\n",
...
static void LIBUSB_CALL ftdi_write_data_cb(struct libusb_transfer *transfer) {
  ...
  if (tc->offset == tc->size) {
    ...
    if (tc->offset + write_size > tc->size) write_size = tc->size - tc->offset;
...
struct ftdi_transfer_control *ftdi_write_data_submit(struct ftdi_context *ftdi,
  ...
  if (ftdi == NULL || ftdi->usb_dev == NULL) return NULL;
...
struct ftdi_transfer_control *ftdi_read_data_submit(struct ftdi_context *ftdi,
  ...
  libusb_fill_bulk_transfer(transfer, ftdi->usb_dev, ftdi->out_ep,
                            ...
                            ftdi_read_data_cb, tc, ftdi->usb_read_timeout);
  transfer->type = LIBUSB_TRANSFER_TYPE_BULK;
...
int ftdi_transfer_data_done(struct ftdi_transfer_control *tc) {
  ...
  while (!tc->completed) {
    ...
    if (ret < 0) {
      ...
      while (!tc->completed)
        if (libusb_handle_events(tc->ftdi->usb_ctx) < 0) break;
   ...
   **/
  if (tc->transfer) {
    if (tc->transfer->status != LIBUSB_TRANSFER_COMPLETED) ret = -1;
    libusb_free_transfer(tc->transfer);
  }
  free(tc);
  return ret;
...
int ftdi_write_data_set_chunksize(struct ftdi_context *ftdi,
  ...
  if (ftdi == NULL) ftdi_error_return(-1, "ftdi context invalid");
...
int ftdi_read_data(struct ftdi_context *ftdi, unsigned char *buf, int size) {
  ...
  while (offset < size && actual_length > 0) {
    ...
    ret = libusb_bulk_transfer(ftdi->usb_dev, ftdi->out_ep, ftdi->readbuffer,
                               ftdi->readbuffer_chunksize, &actual_length,
                               ftdi->usb_read_timeout);
    if (ret < 0) ftdi_error_return(ret, "usb bulk read failed");
...
int ftdi_read_pins(struct ftdi_context *ftdi, unsigned char *pins) {
  ...
  if (libusb_control_transfer(
          ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE, SIO_READ_PINS_REQUEST, 0,
          ftdi->index, (unsigned char *)pins, 1, ftdi->usb_read_timeout) != 1)
...
int ftdi_set_latency_timer(struct ftdi_context *ftdi, unsigned char latency) {
  ...
  usb_val = latency;
  if (libusb_control_transfer(
          ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE, SIO_SET_LATENCY_TIMER_REQUEST,
          usb_val, ftdi->index, NULL, 0, ftdi->usb_write_timeout) < 0)
...
int ftdi_poll_modem_status(struct ftdi_context *ftdi, unsigned short *status) {
  ...
  if (libusb_control_transfer(ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE,
                              SIO_POLL_MODEM_STATUS_REQUEST, 0, ftdi->index,
                              (unsigned char *)usb_val, 2,
...
int ftdi_setflowctrl(struct ftdi_context *ftdi, int flowctrl) {
  ...
  if (libusb_control_transfer(
          ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE, SIO_SET_FLOW_CTRL_REQUEST, 0,
          (flowctrl | ftdi->index), NULL, 0, ftdi->usb_write_timeout) < 0)
...
int ftdi_set_event_char(struct ftdi_context *ftdi, unsigned char eventch,
  ...
  if (libusb_control_transfer(ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
                              SIO_SET_EVENT_CHAR_REQUEST, usb_val, ftdi->index,
                              NULL, 0, ftdi->usb_write_timeout) < 0)
...
int ftdi_set_error_char(struct ftdi_context *ftdi, unsigned char errorch,
  ...
  if (libusb_control_transfer(ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
                              SIO_SET_ERROR_CHAR_REQUEST, usb_val, ftdi->index,
                              NULL, 0, ftdi->usb_write_timeout) < 0)
...
int ftdi_eeprom_initdefaults(struct ftdi_context *ftdi, char *manufacturer,
  ...
  if (ftdi == NULL) ftdi_error_return(-1, "No struct ftdi_context");

  if (ftdi->eeprom == NULL) ftdi_error_return(-2, "No struct ftdi_eeprom");
  ...
  if (eeprom->manufacturer) free(eeprom->manufacturer);
  eeprom->manufacturer = NULL;
  if (manufacturer) {
    eeprom->manufacturer = malloc(strlen(manufacturer) + 1);
    if (eeprom->manufacturer) strcpy(eeprom->manufacturer, manufacturer);
  }

  if (eeprom->product) free(eeprom->product);
  eeprom->product = NULL;
  if (product) {
    eeprom->product = malloc(strlen(product) + 1);
    if (eeprom->product) strcpy(eeprom->product, product);
  } else {
    ...
    switch (ftdi->type) {
      ...
      case TYPE_BM:
        ...
        break;
      case TYPE_2232C:
        default_product = "Dual RS232";
        break;
      case TYPE_R:
        default_product = "FT232R USB UART";
        break;
      case TYPE_2232H:
        default_product = "Dual RS232-HS";
        break;
      case TYPE_4232H:
        default_product = "FT4232H";
        break;
      case TYPE_232H:
        default_product = "Single-RS232-HS";
        break;
      case TYPE_230X:
        default_product = "FT230X Basic UART";
        break;
      default:
        ftdi_error_return(-3, "Unknown chip type");
    }
    eeprom->product = malloc(strlen(default_product) + 1);
    if (eeprom->product) strcpy(eeprom->product, default_product);
  }
  ...
  if (serial) {
    ...
    if (eeprom->serial) strcpy(eeprom->serial, serial);
  }
  ...
  if (ftdi->type == TYPE_R) {
    ...
    if (ftdi->type == TYPE_232H) {
      int i;
      for (i = 0; i < 10; i++) eeprom->cbus_function[i] = CBUSH_TRISTATE;
    }
    eeprom->size = -1;
  }
  switch (ftdi->type) {
...
int ftdi_eeprom_build(struct ftdi_context *ftdi) {
  ...
  if (ftdi == NULL) ftdi_error_return(-2, "No context");
  if (ftdi->eeprom == NULL) ftdi_error_return(-2, "No eeprom structure");
  ...
  if (eeprom->product != NULL) product_size = strlen(eeprom->product);
  if (eeprom->serial != NULL) serial_size = strlen(eeprom->serial);
  ...
  if (user_area_size < 0) ftdi_error_return(-1, "eeprom size exceeded");
  ...
  // Bit 4-0: reserved - 0
  j = 0x80;
  if (eeprom->self_powered) j |= 0x40;
  if (eeprom->remote_wakeup) j |= 0x20;
  output[0x08] = j;
  ...
  if ((ftdi->type != TYPE_AM) && (ftdi->type != TYPE_230X)) {
    ...
    //
    j = 0;
    if (eeprom->in_is_isochronous) j = j | 1;
    if (eeprom->out_is_isochronous) j = j | 2;
    output[0x0A] = j;
  }
  ...
  if (ftdi->type > TYPE_BM) {
    ...
    output[i & eeprom_size_mask] =
        eeprom->is_not_pnp; /* as seen when written with FTD2XX */
    i++;
  }
  ...
  switch (ftdi->type) {
    ...
    case TYPE_2232H:
      ...
      if (eeprom->group0_schmitt == IS_SCHMITT) output[0x0c] |= IS_SCHMITT;
      if (eeprom->group0_slew == SLOW_SLEW) output[0x0c] |= SLOW_SLEW;

      if (eeprom->group1_drive > DRIVE_16MA)
        output[0x0c] |= DRIVE_16MA << 4;
      else
        output[0x0c] |= eeprom->group1_drive << 4;
      if (eeprom->group1_schmitt == IS_SCHMITT) output[0x0c] |= IS_SCHMITT << 4;
      if (eeprom->group1_slew == SLOW_SLEW) output[0x0c] |= SLOW_SLEW << 4;

      if (eeprom->group2_drive > DRIVE_16MA)
        output[0x0d] |= DRIVE_16MA;
      else
        output[0x0d] |= eeprom->group2_drive;
      if (eeprom->group2_schmitt == IS_SCHMITT) output[0x0d] |= IS_SCHMITT;
      if (eeprom->group2_slew == SLOW_SLEW) output[0x0d] |= SLOW_SLEW;

      if (eeprom->group3_drive > DRIVE_16MA)
        output[0x0d] |= DRIVE_16MA << 4;
      else
        output[0x0d] |= eeprom->group3_drive << 4;
      if (eeprom->group3_schmitt == IS_SCHMITT) output[0x0d] |= IS_SCHMITT << 4;
      if (eeprom->group3_slew == SLOW_SLEW) output[0x0d] |= SLOW_SLEW << 4;
    ...
    case TYPE_232H:
      ...
      if (eeprom->group1_schmitt == IS_SCHMITT) output[0x0d] |= IS_SCHMITT;
      if (eeprom->group1_slew == SLOW_SLEW) output[0x0d] |= SLOW_SLEW;
    ...
    case TYPE_230X:
      ...
      output[0x0c] =
          0; /* DBUS drive 4mA, CBUS drive 4 mA like factory default */
      for (j = 0; j <= 6; j++) {
 ...
 * EEPROM structure
 *
 * FTD2XX doesn't allow to set multiple bits in the interface mode bitfield, and
 * so do we
 */
static unsigned char bit2type(unsigned char bits) {
  switch (bits) {
    case 0:
      return CHANNEL_IS_UART;
    case 1:
      return CHANNEL_IS_FIFO;
    case 2:
      return CHANNEL_IS_OPTO;
    case 4:
      return CHANNEL_IS_CPU;
    case 8:
      return CHANNEL_IS_FT1284;
    default:
...
static void print_inverted_bits(int invert) {
  ...
  for (i = 0; i < 8; i++)
    if ((invert & (1 << i)) == (1 << i)) fprintf(stdout, " %s", r_bits[i]);
...
int ftdi_eeprom_decode(struct ftdi_context *ftdi, int verbose) {
  ...
  if (ftdi == NULL) ftdi_error_return(-1, "No context");
  if (ftdi->eeprom == NULL) ftdi_error_return(-1, "No eeprom structure");
  ...
  if (verbose) {
    ...
    if (eeprom->product) fprintf(stdout, "Product:      %s\n", eeprom->product);
    if (eeprom->serial) fprintf(stdout, "Serial:       %s\n", eeprom->serial);
    fprintf(stdout, "Checksum      : %04x\n", checksum);
    if (ftdi->type == TYPE_R)
      fprintf(stdout, "Internal EEPROM\n");
    else if (eeprom->chip >= 0x46)
      fprintf(stdout, "Attached EEPROM: 93x%02x\n", eeprom->chip);
    if (eeprom->suspend_dbus7) fprintf(stdout, "Suspend on DBUS7\n");
    if (eeprom->suspend_pull_downs)
    ...
    if ((ftdi->type == TYPE_2232H) || (ftdi->type == TYPE_4232H)) {
      ...
      char *cbush_mux[] = {
          ...
          "I2C_TXE#",   "I2C_RXF#", "VBUS_SENSE", "BB_WR#",     "BBRD#",
          "TIME_STAMP", "AWAKE#",
      };
      ...
      if (eeprom->invert) print_inverted_bits(eeprom->invert);
    }
...
int ftdi_get_eeprom_value(struct ftdi_context *ftdi,
                          enum ftdi_eeprom_value value_name, int *value) {
  switch (value_name) {
    case VENDOR_ID:
...
int ftdi_get_eeprom_buf(struct ftdi_context *ftdi, unsigned char *buf,
  ...
  if (size > FTDI_MAX_EEPROM_SIZE) size = FTDI_MAX_EEPROM_SIZE;
...
int ftdi_read_eeprom(struct ftdi_context *ftdi) {
  ...
  for (i = 0; i < FTDI_MAX_EEPROM_SIZE / 2; i++) {
    if (libusb_control_transfer(ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE,
                                SIO_READ_EEPROM_REQUEST, 0, i, buf + (i * 2), 2,
                                ftdi->usb_read_timeout) != 2)
      ftdi_error_return(-1, "reading eeprom failed");
  }

  if (ftdi->type == TYPE_R) ftdi->eeprom->size = 0x80;
  /*    Guesses size of eeprom by comparing halves
        - will not work with blank eeprom */
  else if (strrchr((const char *)buf, 0xff) ==
           ((const char *)buf + FTDI_MAX_EEPROM_SIZE - 1))
    ftdi->eeprom->size = -1;
...
int ftdi_read_chipid(struct ftdi_context *ftdi, unsigned int *chipid) {
  ...
  if (libusb_control_transfer(
          ...
          0x43, (unsigned char *)&a, 2, ftdi->usb_read_timeout) == 2) {
    a = a << 8 | a >> 8;
    if (libusb_control_transfer(
            ftdi->usb_dev, FTDI_DEVICE_IN_REQTYPE, SIO_READ_EEPROM_REQUEST, 0,
            0x44, (unsigned char *)&b, 2, ftdi->usb_read_timeout) == 2) {
      b = b << 8 | b >> 8;
      a = (a << 16) | (b & 0xFFFF);
...
int ftdi_erase_eeprom(struct ftdi_context *ftdi) {
  ...
  /* detect chip type by writing 0x55AA as magic at word position 0xc0
     Chip is 93x46 if magic is read at word position 0x00, as wraparound happens
     around 0x40
     Chip is 93x56 if magic is read at word position 0x40, as wraparound happens
     around 0x80
     Chip is 93x66 if magic is only read at word position 0xc0*/
  if (libusb_control_transfer(ftdi->usb_dev, FTDI_DEVICE_OUT_REQTYPE,
                              SIO_WRITE_EEPROM_REQUEST, MAGIC, 0xc0, NULL, 0,
                              ftdi->usb_write_timeout) != 0)
