...
#define MAX_OPTION 14
  const char *options[MAX_OPTION] = {
      "TXDEN",   "PWREN",      "RXLED",      "TXLED",  "TXRXLED",
 ...
 **/
static void eeprom_set_value(struct ftdi_context *ftdi,
 ...
 * Function will abort the program on error
 **/
static void eeprom_get_value(struct ftdi_context *ftdi,
                             enum ftdi_eeprom_value value_name, int *value) {
...
int main(int argc, char *argv[]) {
  ...
  */
  cfg_opt_t opts[] = {
      CFG_INT("vendor_id", 0, 0), CFG_INT("product_id", 0, 0),
      ...
      CFG_INT("usb_version", 0, 0), CFG_INT("default_pid", 0x6001, 0),
      CFG_INT("max_power", 0, 0), CFG_STR("manufacturer", "Acme Inc.", 0),
      CFG_STR("product", "USB Serial Converter", 0),
      CFG_STR("serial", "08-15", 0), CFG_INT("eeprom_type", 0x00, 0),
      CFG_STR("filename", "", 0), CFG_BOOL("flash_raw", cfg_false, 0),
      CFG_BOOL("high_current", cfg_false, 0),
      CFG_STR_LIST("cbus0",
                   "{TXDEN,PWREN,RXLED,TXLED,TXRXLED,SLEEP,CLK48,CLK24,CLK12,"
                   "CLK6,IO_MODE,BITBANG_WR,BITBANG_RD,SPECIAL}",
                   0),
      CFG_STR_LIST("cbus1",
                   "{TXDEN,PWREN,RXLED,TXLED,TXRXLED,SLEEP,CLK48,CLK24,CLK12,"
                   "CLK6,IO_MODE,BITBANG_WR,BITBANG_RD,SPECIAL}",
      ...
      CFG_STR_LIST(
          "cbus4",
          "{TXDEN,PWRON,RXLED,TXLED,TX_RX_LED,SLEEP,CLK48,CLK24,CLK12,CLK6}",
          0),
      ...
      CFG_BOOL("invert_dcd", cfg_false, 0), CFG_BOOL("invert_ri", cfg_false, 0),
      CFG_END()};
  cfg_t *cfg;
  ...
  */
  ...
  if (argc != 2 && argc != 3) {
    ...
    printf(
        "--read-eeprom  Read eeprom and write to -filename- from "
        "config-file\n");
  ...
  if (_read > 0 || _erase > 0 || _flash > 0) {
    ...
    if (i != 0) {
      ...
      printf(
          "Unable to find FTDI devices under given vendor/product id: "
          "0x%X/0x%X\n",
  ...
  eeprom_set_value(ftdi, IN_IS_ISOCHRONOUS,
                   cfg_getbool(cfg, "in_is_isochronous"));
  eeprom_set_value(ftdi, OUT_IS_ISOCHRONOUS,
                   cfg_getbool(cfg, "out_is_isochronous"));
  eeprom_set_value(ftdi, SUSPEND_PULL_DOWNS,
                   cfg_getbool(cfg, "suspend_pull_downs"));

  eeprom_set_value(ftdi, USE_SERIAL, cfg_getbool(cfg, "use_serial"));
  eeprom_set_value(ftdi, USE_USB_VERSION,
                   cfg_getbool(cfg, "change_usb_version"));
  ...
  eeprom_set_value(ftdi, CBUS_FUNCTION_0,
                   str_to_cbus(cfg_getstr(cfg, "cbus0"), 13));
  eeprom_set_value(ftdi, CBUS_FUNCTION_1,
                   str_to_cbus(cfg_getstr(cfg, "cbus1"), 13));
  eeprom_set_value(ftdi, CBUS_FUNCTION_2,
                   str_to_cbus(cfg_getstr(cfg, "cbus2"), 13));
  eeprom_set_value(ftdi, CBUS_FUNCTION_3,
                   str_to_cbus(cfg_getstr(cfg, "cbus3"), 13));
  eeprom_set_value(ftdi, CBUS_FUNCTION_4,
  ...
  if (size_check == -1) {
    printf(
        "Sorry, the eeprom can only contain 128 bytes (100 bytes for your "
        "strings).\n");
  ...
  if (filename != NULL && strlen(filename) > 0 &&
    ...
    if (eeprom_buf == NULL) eeprom_buf = malloc(my_eeprom_size);
    ftdi_get_eeprom_buf(ftdi, eeprom_buf, my_eeprom_size);
