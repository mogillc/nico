...
void Motor::buildRegisterMap() {
	if (registerMap.size() == 0) {
		{  // XL320:
			...
			registerMap[MODEL_XL320][REG_Ret_Delay_Time] =
			REG_XL_Ret_Delay_Time;
			...
			registerMap[MODEL_XL320][REG_Voltage_Lim_Low] =
			REG_XL_Voltage_Lim_Low;
			registerMap[MODEL_XL320][REG_Voltage_Lim_Hi] =
			REG_XL_Voltage_Lim_Hi;
			registerMap[MODEL_XL320][REG_Torque_Max] = REG_XL_Torque_Max;
			registerMap[MODEL_XL320][REG_Stat_Ret_Level] =
			REG_XL_Stat_Ret_Level;
			registerMap[MODEL_XL320][REG_Alarm_Shutdown] =
			REG_XL_Alarm_Shutdown;
			...
			registerMap[MODEL_XL320][REG_Present_Voltage] =
			REG_XL_Present_Voltage;
		...
		{  // MX-series:
			...
			for (int i = 0; i < sizeof(MX_series) / sizeof(*MX_series); i++) {
				registerMap[MX_series[i]][REG_Model_Number] =
				REG_MX_Model_Number;
				registerMap[MX_series[i]][REG_Firmware_Ver] =
				REG_MX_Firmware_Ver;
				registerMap[MX_series[i]][REG_ID] = REG_MX_ID;
				registerMap[MX_series[i]][REG_Baud] = REG_MX_Baud;
				registerMap[MX_series[i]][REG_Ret_Delay_Time] =
				REG_MX_Ret_Delay_Time;
				...
				registerMap[MX_series[i]][REG_Voltage_Lim_Low] =
				REG_MX_Voltage_Lim_Low;
				registerMap[MX_series[i]][REG_Voltage_Lim_Hi] =
				REG_MX_Voltage_Lim_Hi;
				registerMap[MX_series[i]][REG_Torque_Max] = REG_MX_Torque_Max;
				registerMap[MX_series[i]][REG_Stat_Ret_Level] =
				REG_MX_Stat_Ret_Level;
				registerMap[MX_series[i]][REG_Alarm_LED] = REG_MX_Alarm_LED;
				registerMap[MX_series[i]][REG_Alarm_Shutdown] =
				REG_MX_Alarm_Shutdown;
				registerMap[MX_series[i]][REG_Multi_Turn_Offset] =
				REG_MX_Multi_Turn_Offset;
				registerMap[MX_series[i]][REG_Resolution_Divider] =
				REG_MX_Resolution_Divider;
				...
				registerMap[MX_series[i]][REG_Present_Speed] =
				REG_MX_Present_Speed;
				registerMap[MX_series[i]][REG_Present_Load] =
				REG_MX_Present_Load;
				registerMap[MX_series[i]][REG_Present_Voltage] =
				REG_MX_Present_Voltage;
				registerMap[MX_series[i]][REG_Present_Temp] =
				REG_MX_Present_Temp;
				...
				registerMap[MX_series[i]][REG_Goal_Acceleration] =
				REG_MX_Goal_Acceleration;
			}
			registerMap[MODEL_MX106][REG_Drive_Mode] = REG_MX106_Drive_Mode;
			registerMap[MODEL_MX106][REG_Current] = REG_MX106_Current;
			registerMap[MODEL_MX106][REG_Torque_Mode_EN] =
			REG_MX106_Torque_Mode_EN;
		...
		{  // AX/RX/DX/EX-series:
			...
			for (int i = 0; i < sizeof(AX_series) / sizeof(*AX_series); i++) {
				...
				REG_AX_Model_Number;
				registerMap[AX_series[i]][REG_Firmware_Ver] =
				REG_AX_Firmware_Ver;
				registerMap[AX_series[i]][REG_ID] = REG_AX_ID;
				registerMap[AX_series[i]][REG_Baud] = REG_AX_Baud;
				registerMap[AX_series[i]][REG_Ret_Delay_Time] =
				REG_AX_Ret_Delay_Time;
				...
				registerMap[AX_series[i]][REG_Voltage_Lim_Low] =
				REG_AX_Voltage_Lim_Low;
				registerMap[AX_series[i]][REG_Voltage_Lim_Hi] =
				REG_AX_Voltage_Lim_Hi;
				registerMap[AX_series[i]][REG_Torque_Max] = REG_AX_Torque_Max;
				registerMap[AX_series[i]][REG_Stat_Ret_Level] =
				REG_AX_Stat_Ret_Level;
				registerMap[AX_series[i]][REG_Alarm_LED] = REG_AX_Alarm_LED;
				registerMap[AX_series[i]][REG_Alarm_Shutdown] =
				REG_AX_Alarm_Shutdown;
				...
				registerMap[AX_series[i]][REG_Present_Speed] =
				REG_AX_Present_Speed;
				registerMap[AX_series[i]][REG_Present_Load] =
				REG_AX_Present_Load;
				registerMap[AX_series[i]][REG_Present_Voltage] =
				REG_AX_Present_Voltage;
				registerMap[AX_series[i]][REG_Present_Temp] =
				REG_AX_Present_Temp;
		...
		{  // Pro-series:
			...
			for (int i = 0; i < sizeof(AX_series) / sizeof(*AX_series); i++) {
				registerMap[AX_series[i]][REG_Model_Number] =
				REG_PRO_Model_Number;
				registerMap[AX_series[i]][REG_Model_Info] = REG_PRO_Model_Info;
				registerMap[AX_series[i]][REG_Firmware_Ver] =
				REG_PRO_Firmware_Ver;
				registerMap[AX_series[i]][REG_ID] = REG_PRO_ID;
				registerMap[AX_series[i]][REG_Baud] = REG_PRO_Baud;
				registerMap[AX_series[i]][REG_Ret_Delay_Time] =
				REG_PRO_Ret_Delay_Time;
				registerMap[AX_series[i]][REG_Operating_Mode] =
				REG_PRO_Operating_Mode;
				registerMap[AX_series[i]][REG_Homing_Offset] =
				REG_PRO_Homing_Offset;
				registerMap[AX_series[i]][REG_Moving_Thresh] =
				REG_PRO_Moving_Thresh;
				registerMap[AX_series[i]][REG_Temp_Lim] = REG_PRO_Temp_Lim;
				registerMap[AX_series[i]][REG_Voltage_Lim_Hi] =
				REG_PRO_Voltage_Lim_Hi;
				registerMap[AX_series[i]][REG_Voltage_Lim_Low] =
				REG_PRO_Voltage_Lim_Low;
				registerMap[AX_series[i]][REG_Accelerate_Lim] =
				REG_PRO_Accelerate_Lim;
				registerMap[AX_series[i]][REG_Torque_Max] = REG_PRO_Torque_Max;
				registerMap[AX_series[i]][REG_Velocity_Lim] =
				REG_PRO_Velocity_Lim;
				registerMap[AX_series[i]][REG_Max_Position_Lim] =
				REG_PRO_Max_Position_Lim;
				registerMap[AX_series[i]][REG_Min_Position_Lim] =
				REG_PRO_Min_Position_Lim;
				registerMap[AX_series[i]][REG_Ext_Port_Mode_1] =
				REG_PRO_Ext_Port_Mode_1;
				registerMap[AX_series[i]][REG_Ext_Port_Mode_2] =
				REG_PRO_Ext_Port_Mode_2;
				registerMap[AX_series[i]][REG_Ext_Port_Mode_3] =
				REG_PRO_Ext_Port_Mode_3;
				registerMap[AX_series[i]][REG_Ext_Port_Mode_4] =
				REG_PRO_Ext_Port_Mode_4;
				registerMap[AX_series[i]][REG_Alarm_Shutdown] =
				REG_PRO_Shutdown;
				//					for (int x = 1; x <= 256; x++) {
				...
				registerMap[AX_series[i]][REG_Goal_Torque] =
				REG_PRO_Goal_Torque;
				registerMap[AX_series[i]][REG_Goal_Acceleration] =
				REG_PRO_Goal_Acceleration;
				registerMap[AX_series[i]][REG_Moving] = REG_PRO_Moving;
				registerMap[AX_series[i]][REG_Present_Pos] =
				REG_PRO_Present_Pos;
				registerMap[AX_series[i]][REG_Present_Speed] =
				REG_PRO_Present_Speed;
				registerMap[AX_series[i]][REG_Current] =
				REG_PRO_Present_Current;
				registerMap[AX_series[i]][REG_Present_Voltage] =
				REG_PRO_Present_Voltage;
				registerMap[AX_series[i]][REG_Present_Temp] =
				REG_PRO_Present_Temp;
				registerMap[AX_series[i]][REG_Ext_Port_Data_1] =
				REG_PRO_Ext_Port_Data_1;
				registerMap[AX_series[i]][REG_Ext_Port_Data_2] =
				REG_PRO_Ext_Port_Data_2;
				registerMap[AX_series[i]][REG_Ext_Port_Data_3] =
				REG_PRO_Ext_Port_Data_3;
				registerMap[AX_series[i]][REG_Ext_Port_Data_4] =
				REG_PRO_Ext_Port_Data_4;
				//					registerMap[AX_series[i]][REG_Model_Number]
				//= REG_PRO_Indirect_Data(x)	((x)+633)	// up to 256
				registerMap[AX_series[i]][REG_Registered] = REG_PRO_Registered;
				registerMap[AX_series[i]][REG_Stat_Ret_Level] =
				REG_PRO_Stat_Ret_Level;
				registerMap[AX_series[i]][REG_Hardware_Error] =
				REG_PRO_Hard_Error;
			}
		}
	}
