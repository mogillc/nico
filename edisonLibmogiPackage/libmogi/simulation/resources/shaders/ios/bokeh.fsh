...
// Note: all texture2D() methods replaced with texture2D() for ES conversion
...
//layout(location = 0) out vec4 FragColor;	// TODO: check implementation

vec3 color(vec2 coords,float blur) //processing the sample
{
	vec3 col = vec3(0.0);

	col.r = texture2D( bgl_RenderedTexture, coords + vec2( 0.0, 1.0) * fringe * blur).r;
	col.g = texture2D( bgl_RenderedTexture, coords + vec2( -0.866, -0.5) * fringe * blur).g;
	col.b = texture2D( bgl_RenderedTexture, coords + vec2( 0.866, -0.5) * fringe * blur).b;
...
void main()
{
	...
	float depth = linearize(texture2D(bgl_DepthTexture, TexCoord[0].st ).x);
	...
	col = texture2D(bgl_RenderedTexture, TexCoord[0].st).rgb;
	float s = 1.0;
	int ringsamples;
	...
	//vec4 bgl_FragColor = texture2D( bgl_RenderedTexture, pos);
